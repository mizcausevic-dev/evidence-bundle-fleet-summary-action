import { REGULATED_PURPOSES } from "./types.js";
...
export function summarize(manifests, opts = {}) {
    ...
    for (const m of manifests) {
        if (!m.bundle || !Array.isArray(m.items))
            continue;
        const id = m.bundle.id;
        const purpose = m.bundle.purpose ?? "unspecified";
        ...
        for (const i of m.items) {
            ...
            if (set)
                ...
                hashIndex.set(i.sha256, new Set([id]));
        }
    }
